...
const ExercisesScreen = () => {
  ...
  const handleFinish = async () => {
    try {
      const sessionData = {
        user_id: "PXDYJCKtnULevYyzaNgp", // Replace with the actual user ID
        workout_id: workoutData.id,
        date: new Date(),
        exercises: workoutData.exercises.map((exercise) => {
          return {
            exercise_id: exercise,
            sets: exerciseSets[exercise.id].map((set) => ({
              weight: set.weight,
              reps: set.reps,
              rpe: set.rpe,
      ...
      // Add the session data to Firestore
      const sessionsCollection = collection(db, "sessions");
      await addDoc(sessionsCollection, sessionData);

      console.log("Workout finished and saved to Firebase!");
